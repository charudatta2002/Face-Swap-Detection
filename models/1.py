...
if __name__ == "__main__":
    ...
    for label in labels:
        ...
        X = np.load(os.path.join(output_path, f"X_{label}.npy"))
        y = np.load(os.path.join(output_path, f"y_{label}.npy"))
    ...
    num_frames = 450
    num_features = 136
    X_train_reshaped = X_train.reshape(X_train.shape[0], num_frames, num_features, 1)
    X_test_reshaped = X_test.reshape(X_test.shape[0], num_frames, num_features, 1)

    # Build CNN model
    model = Sequential([
        Conv2D(32, (3, 3), activation='relu', input_shape=(num_frames, num_features, 1)),
        MaxPooling2D((2, 2)),
        Flatten(),
